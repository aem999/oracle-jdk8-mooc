...
//        .map(s -> wordList.stream().mapToInt(k -> Levenshtein.lev(s, k)).toArray())
//        .toArray(int[][]::new);

    (parallel ? wordList.parallelStream() : wordList.stream())
        .map(w1 -> wordList.stream().mapToInt(w2 -> Levenshtein.lev(w1, w2)).toArray())
        .collect(Collectors.toList())
        .toArray(distances);

    // IntStream.range(0, LIST_SIZE).forEachOrdered(i -> System.out.println(i + ": " + Arrays.toString(distances[i])));
    return distances;
  }
